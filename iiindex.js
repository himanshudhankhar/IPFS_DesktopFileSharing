...
event.preventDefault();
ipcRenderer.send('OpenVideo',  {hash:file.hash,name:file.name});
  ...
  // alert(message[0].name);
   ...
   document.getElementById("lawda").addEventListener("click",function(e) {
    ...
    //now we got hash we can find its name from files array
      ...
      ipcRenderer.send('OpenVideo', {name:fname,hash:hash } );
      ipcRenderer.send('DownloadFile',  {hash:hash,name:fname});
      }
    });
...
//    ipcRenderer.send('OpenVideo', Hash  );
